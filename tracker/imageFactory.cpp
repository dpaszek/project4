...
ImageFactory::~ImageFactory() {
  ...
  while(surfPtr != surfaces.end())
  {
  	SDL_FreeSurface(surfPtr->second);
  	++surfPtr;
  }

  std::map<std::string, SDL_Texture*>::const_iterator 
  	textPtr = textures.begin();
  while(textPtr != textures.end())
  {
  	SDL_DestroyTexture(textPtr->second);
  	++textPtr;
  }
